...
public class DAGStar implements GraphTraversalAlgorithm {
    /**
    ...
    private OperatorCostEstimatorInterface costEstimator;
    private int timeout;
    private OptimizationRequestStatisticsBundle stats;
    private Logger logger;
    private ExecutorService executorService;
    private final ScheduledExecutorService statisticsExecutorService;
    ...
    public DAGStar(AggregationStrategy aggregationStrategy, boolean enableStatisticsOutput) {
    ...
    public void setup(OptimizationResourcesBundle bundle, BoundedPriorityQueue<OptimizationPlan> validPlans, OptimizationPlan rootPlan,
        ...
        Network inforeNetwork = bundle.getNetwork();
...
//        }
    ...
    private void explore(Map<String, Set<String>> operatorChildrenOriginal,
                         ...
                         OperatorCostEstimatorInterface costEstimator,
...
//            }
            ...
            if (currentPlan.containsOperator(VIRTUAL_END)) {
                ...
                System.out.println("Found a complete plan with cost: " + completePlan.totalCost() / (double) OperatorDagStarCostEstimator.COST_MULTIPLIER);
                System.out.println();
                System.out.println("Time taken: " + (endTime - startTime) + " ms");
                System.out.println("Plans explored: " + planCount);
                // Print the operators and their implementations;
                System.out.println("Operators and implementations:");
                completePlan.getOperatorsAndImplementations().forEach((op, impl) -> System.out.println("Operator: " + op + ", Implementation: " + impl._1 + ", Platform: " + impl._2));
                System.out.println("-----------------------------------------------------------");
                System.out.println();
...
//            System.out.println("Creating " + availableImplementations.size() + " new plans for operator " + newOperator);
            ...
            for (Tuple<String, String> availableImplementation : availableImplementations) {
                ...
                if (aggregationStrategy.equals(AggregationStrategy.SUM)) {
                    ...
                    int newOperatorProvisionalCost = costEstimator.getOperatorAndImplementationCost(newOperator, availableImplementation);
                    if (newOperatorProvisionalCost == Integer.MAX_VALUE) {
                        continue;
                    }

                    newOperatorRealCost =  newOperatorProvisionalCost + maxRealPlusCommCost;
                }
...
//                System.out.println("Inserting new plan with " + newImplementations.size() + " with cost: " + " | gCost: " + gCost + " | hCost: " + hCost);
    ...
    /**
     * Selects an operator currently not in the polled solution, but all
     ...
     */
    private Map<String, Integer> calculateHeuristics(OperatorCostEstimatorInterface costEstimator,
                                                     Map<String, Set<String>> children,
                                                     Map<String, Set<String>> parents,
                                                     String virtualEndNode,
                                                     String virtualStartNode) {
