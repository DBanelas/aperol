...
public abstract class AbstractPlanBasedAlgorithm implements PlanBasedAlgorithm {
    ...
    /**
     * Method to initialize the class's necessary parts
    ...
    protected void scheduleTimeout() {
        timeoutExecutor.schedule(() -> {
            System.out.println("\nTimeout reached after " + timeout + " ms. Stopping the algorithm.");
            timeoutReached = true;
    ...
    public OptimizationResult aggregateResults() {
        ...
        while (terminationPredicate.test(completedTasks) && !timeoutReached) {
            try {
                Future<Tuple<Graph, Integer>> future = completionService.poll(20, TimeUnit.MILLISECONDS);
                if (future == null) {
                    continue; // No task completed yet, continue polling
                }
        ...
        System.out.println("Timeout is: " + timeoutReached);
