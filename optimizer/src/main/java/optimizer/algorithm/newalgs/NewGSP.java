...
public class NewGSP {
    ...
    private static void scheduleStatistics(ScheduledExecutorService statisticsExecutor,
        ...
        statisticsExecutor.scheduleAtFixedRate(() -> {
            System.out.print("\rCompleted tasks: " + visited +
                    ...
                    " | " + "Best score: " + bestCost +
                    " | " + "Failed sims: " + SimulationPlanCostEstimator.failedSimulations);
    ...
    public static Graph createPlanSpaceGSProgressive(Graph flow,
        ...
        sortedActions.sort(Comparator.comparingInt(t -> t._2));
        ...
        final List<Tuple<Integer, Integer>> actionsToPassFinal = new ArrayList<>(actionsToPass);
...
//        printInfoBeforeExecution(operators, platforms, sites, possiblePlans, actions);
        ...
        Future<?> task = executorService.submit(() -> checkAction(new Graph(flow), actionsToPassFinal, cloudOnlyOperatorIds, bestPlanRef));
        ...
        while (!vertexList.isEmpty()) {
            ...
            for (Vertex v : vertexList) {
                ...
                for (Tuple<Integer, Integer> a : actions) {
                    ...
                    // keeps the least expensive action a per vertex v
                    if (costEstimation instanceof PlanDagStarCostEstimator) {
