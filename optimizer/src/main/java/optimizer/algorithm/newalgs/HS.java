...
import optimizer.prune.GraphPlanPruner;
import optimizer.prune.VaryWeightsSkylinePlanPruner;
...
public class HS {
    ...
    public void setup() {
        ...
        GraphPlanPruner pruner;
        if (costEstimation instanceof WeightedFitIoTCostEstimator) pruner = new VaryWeightsSkylinePlanPruner(this.costEstimation);
        else pruner = new DynamicSkylinePlanPruner(this.costEstimation);
...
//            System.out.println("Best plan cost: " + bestPlan.getCost());
        } else if (costEstimation instanceof WeightedFitIoTCostEstimator) {
            bestPlan =  Collections.max(planGraph, Comparator.comparingInt(Graph::getCost));
//            System.out.println("Best plan cost: " + (bestPlan.getCost() - rootFlow.getCost()));
        } else {
            bestPlan =  Collections.max(planGraph, Comparator.comparingInt(Graph::getCost));
//            System.out.println("Best plan cost: " + (bestPlan.getCost() - rootFlow.getCost()));
        }
//        System.out.println("Best plan signature: " + bestPlan.getSignatureDashed());
//        System.out.println("Best plan graph: \n" + bestPlan);
        return bestPlan;
    }
    ...
    private class HSpTask implements Callable<String> {
        ...
        private final GraphPlanPruner planPruner;
        ...
        public HSpTask(int workerID,
                       ...
                       Set<String> removed,
                       ArrayList<Tuple<Integer, Integer>> actions,
                       PlanCostEstimatorInterface costEstimator,
                       GraphPlanPruner planPruner,
                       CountDownLatch latch) {
...
//                logWorkerCompletion(startInstant);
         ...
         * Checks for a null object and 0 active tasks.
         ...
         * Performs several checks to determine whether the resulting plan has been
         * examined before.
        ...
        private void processAction(Graph parent, Vertex parentVertex, Tuple<Integer, Integer> action) {
            try {
                ...
                // At this point, the candidate graph is valid and not pruned.
         ...
         * Add it to the planGraph with the possible solutions.
         * Add it back to the queue for further expansion
