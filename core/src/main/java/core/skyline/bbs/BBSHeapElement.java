...
public class BBSHeapElement implements Comparable<BBSHeapElement> {
    ...
    public BBSHeapElement(Node<Object, Point> node, boolean isNode) {
        this.node = node;
        // Min distance is calculated using the L1 norm
        this.heapElementMinDist = Math.abs(node.geometry().mbr().x1()) + Math.abs(node.geometry().mbr().y1());
        this.isNode = isNode;
    }
    ...
    public BBSHeapElement(Entry<Object, Point> entry, boolean isNode) {
        this.entry = entry;
        this.heapElementMinDist = Math.abs(entry.geometry().x()) + Math.abs(entry.geometry().y());
        this.isNode = isNode;
    }
